...
# load data
X <- subset(ggs.resp, select=c(country, fifle, hrwkMom.c, 
...
fifle_nomac <- jags(data, inits, model.file = "analyses/model/model_nomac_300615.R",
                    parameters  = c("alpha", 
...
save(fifle_nomac, file="analyses/results/fifle_nomac_160118.RData")
# load("analyses/results/fifle_nomac_160118.RData")

# visual diagnoses
fifle_nomac_mcmc <- as.mcmc(fifle_nomac)
sum <- summary(fifle_nomac_mcmc)
# traceplot(fifle_nomac_mcmc)
mcmcplot(fifle_nomac_mcmc)
...
caterplot(fifle_nomac_mcmc, parms = "beta1") # credible intervals plot (add labels later)
...
# HPD quantiles
quants12 <- HPDinterval(fifle_nomac_mcmc)
quants1 <- quants12[[1]]
quants2 <- quants12[[2]]
...
# graph beta1
png("analyses/results/graph_fifle_nomac_b1_160118.png")
...
analysisresults.fifle_nomac <- as.data.frame(cbind(coefficients, quantiles))
analysisresults.fifle_nomac$var <- c("hrwkMom.Austrlia", "hrwkMom.Austria", "hrwkMom.Belgium", "hrwkMom.Bulgaria", 
...
save(analysisresults.fifle_nomac, file="analyses/results/analysisresults.fifle_nomac_160118.RData")
load("analyses/results/analysisresults.fifle_nomac_160118.RData")
View(analysisresults.fifle_nomac)
