...
# load data
X <- subset(ggs.resp, select=c(country, fifle, hrwkMom.c, gpg.c,
...
fifle_gpg <- jags(data, inits, model.file = "analyses/model/model_mac_300615.R",
                  parameters  = c("alpha", 
...
save(fifle_gpg, file="analyses/results/fifle_gpg_160118.RData")
# load("analyses/results/fifle_gpg_160118.RData")

# visual diagnoses
fifle_gpg_mcmc <- as.mcmc(fifle_gpg)
sum <- summary(fifle_gpg_mcmc)
# traceplot(eqlsmi)
mcmcplot(fifle_gpg_mcmc)
...
caterplot(fifle_gpg_mcmc, parms = "beta1") # credible intervals plot
...
# HPD quantiles
quants12 <- HPDinterval(fifle_gpg_mcmc)
quants1 <- quants12[[1]]
quants2 <- quants12[[2]]
...
# graph beta1
png("analyses/results/graph_fifle_gpg_b1_160118.png")
...
analysisresults.fifle_gpg <- as.data.frame(cbind(coefficients, quantiles))
analysisresults.fifle_gpg$var <- c("hrwkMom.Austrlia", "hrwkMom.Austria", "hrwkMom.Belgium", "hrwkMom.Bulgaria", 
                                   ...
                                   "intercept.country", "gpg", "intercept beta1", 
                                   "hrwkMom * gpg", "level2 var",
                                   "beta1 var", "level1 var")

save(analysisresults.fifle_gpg, file="analyses/results/analysisresults.fifle_gpg_160118.RData")
load("analyses/results/analysisresults.fifle_gpg_160118.RData")
View(analysisresults.fifle_gpg)
