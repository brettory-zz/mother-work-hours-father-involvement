...
# With cross-level interaction of gender pay gap
...
# load data
X <- subset(ggs.resp, select=c(country, fistr, hrwkMom.c, gpg.c,
...
data <- list(ficon = Y$fistr, country = Y$country, hrwkMom = Y$hrwkMom.c, 
             mac = Y$gpg.c, age = Y$maleage.c, edu = Y$maleedu.c, 
...
fistr_gpg <- jags(data, inits, model.file = "analyses/model/model_mac_300615.R",
                  parameters  = c("alpha", 
...
save(fistr_gpg, file="analyses/results/fistr_gpg_160118.RData")
# load("analyses/results/fistr_gpg_160118.RData")

# visual diagnoses
fistr_gpg_mcmc <- as.mcmc(fistr_gpg)
sum <- summary(fistr_gpg_mcmc)
# traceplot(eqlsmi)
mcmcplot(fistr_gpg_mcmc)
...
caterplot(fistr_gpg_mcmc, parms = "beta1") # credible intervals plot
...
# HPD quantiles
quants12 <- HPDinterval(fistr_gpg_mcmc)
quants1 <- quants12[[1]]
quants2 <- quants12[[2]]
...
# graph beta1
png("analyses/results/graph_fistr_gpg_b1_160118.png")
...
analysisresults.fistr_gpg <- as.data.frame(cbind(coefficients, quantiles))
analysisresults.fistr_gpg$var <- c("hrwkMom.Austrlia", "hrwkMom.Austria", "hrwkMom.Belgium", "hrwkMom.Bulgaria", 
                                   ...
                                   "intercept.country", "gender pay gap", "intercept beta1", 
                                   "hrwkMom * gpg", "level2 var",
                                   "beta1 var", "level1 var")

save(analysisresults.fistr_gpg, file="analyses/results/analysisresults.fistr_gpg_160118.RData")
load("analyses/results/analysisresults.fistr_gpg_160118.RData")
View(analysisresults.fistr_gpg)
