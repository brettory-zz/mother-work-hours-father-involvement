...
# load data
X <- subset(ggs.resp, select=c(country, fistr, hrwkMom.c, pat.c,
...
fistr_pat <- jags(data, inits, model.file = "analyses/model/model_mac_300615.R",
                     parameters  = c("alpha", 
...
save(fistr_pat, file="analyses/results/fistr_pat_160118.RData")
# load("analyses/results/fistr_pat_160118.RData")

# visual diagnoses
fistr_pat_mcmc <- as.mcmc(fistr_pat)
sum <- summary(fistr_pat_mcmc)
# traceplot(eqlsmi)
mcmcplot(fistr_pat_mcmc)
...
caterplot(fistr_pat_mcmc, parms = "beta1") # credible intervals plot
...
# HPD quantiles
quants12 <- HPDinterval(fistr_pat_mcmc)
quants1 <- quants12[[1]]
quants2 <- quants12[[2]]
...
save(analysisresults.fistr_pat, file="analyses/results/analysisresults.fistr_pat_160118.RData")
load("analyses/results/analysisresults.fistr_pat_160118.RData")
View(analysisresults.fistr_pat)
