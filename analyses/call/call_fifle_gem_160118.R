...
# load data
X <- subset(ggs.resp, select=c(country, fifle, hrwkMom.c, gem.c,
...
fifle_gem <- jags(data, inits, model.file = "analyses/model/model_mac_300615.R",
                  parameters  = c("alpha", 
...
save(fifle_gem, file="analyses/results/fifle_gem_160118.RData")
# load("analyses/results/fifle_gem_160118.RData")

# visual diagnoses
fifle_gem_mcmc <- as.mcmc(fifle_gem)
sum <- summary(fifle_gem_mcmc)
# traceplot(eqlsmi)
mcmcplot(fifle_gem_mcmc)
...
caterplot(fifle_gem_mcmc, parms = "beta1") # credible intervals plot
...
# HPD quantiles
quants12 <- HPDinterval(fifle_gem_mcmc)
quants1 <- quants12[[1]]
quants2 <- quants12[[2]]
...
# graph beta1
png("analyses/results/graph_fifle_gem_b1_160118.png")
...
analysisresults.fifle_gem <- as.data.frame(cbind(coefficients, quantiles))
analysisresults.fifle_gem$var <- c("hrwkMom.Austrlia", "hrwkMom.Austria", "hrwkMom.Belgium", "hrwkMom.Bulgaria", 
...
save(analysisresults.fifle_gem, file="analyses/results/analysisresults.fifle_gem_160118.RData")
load("analyses/results/analysisresults.fifle_gem_160118.RData")
View(analysisresults.fifle_gem)
